...
Fixes aplicados:
  ...
  - Busca robusta de index.html em múltiplos caminhos
"""
...
async def railway_health():
    """Health check mínimo para Railway — SEM query no banco, responde instantaneamente"""
    return {"status": "healthy", "service": "radar-pericial"}

@app.get("/")
async def root():
    """Rota raiz: serve HTML se existir, senão retorna JSON"""
    # Tenta múltiplos caminhos possíveis para index.html
    possible_paths = [
        ...
        Path("/app/api/../interface/templates/index.html"),
    ]
    ...
    # Fallback: retorna JSON se HTML não for encontrado
...
async def log_requests(request, call_next):
    ...
    # Silencia logs excessivos para health checks
