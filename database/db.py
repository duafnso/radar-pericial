...
Tabelas geoespaciais + tabelas judiciais/administrativas

Fixes aplicados:
  - Engine como singleton (module-level) com pool de conexões
  - _init_schema chamado apenas na inicialização do app (via init_db())
  - bcrypt para hash de senha (com upgrade automático de SHA256 legado)
  - Token de autenticação armazenado na tabela usuarios (válido 24h)
  - save_portarias com deduplicação por (titulo, data_publicacao, fonte)
  - save_movimentacao com checagem de duplicata antes de inserir
...
class Database:
    ...
    def _init_schema(self):
        """Cria extensões e tabelas. Chamado UMA VEZ via init_db()."""
        sql = """
        CREATE EXTENSION IF NOT EXISTS postgis;
        CREATE EXTENSION IF NOT EXISTS pg_trgm;

        -- Usuários (com suporte a token de sessão)
        ...
        -- Parcelas SIGEF / desapropriações
    ...
    def check_login(self, username: str, password_raw: str) -> bool:
        ...
        with self.engine.connect() as conn:
            ...
            if hashlib.sha256(password_raw.encode()).hexdigest() == stored:
                ...
                conn.commit()
                logger.info(f"Senha do usuário '{username}' upgradada para bcrypt.")
                return True
            return False
        ...
        with self.engine.connect() as conn:
            row = conn.execute(
                text(
                    ...
                    "WHERE token=:t AND token_expira > NOW()"
                ),
                {"t": token},
            ).fetchone()
    ...
    def save_geodataframe(self, gdf, table: str, if_exists: str = "append"):
        ...
        if not isinstance(gdf, gpd.GeoDataFrame):
            try:
                pd.DataFrame(gdf).to_sql(
                    table, self.engine, if_exists=if_exists, index=False
                )
        ...
        try:
            ...
            logger.info(f"'{table}': {len(gdf)} registros salvos (if_exists={if_exists})")
        except Exception as e:
    ...
    def _upsert_sigef(self, gdf: gpd.GeoDataFrame):
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                conn.execute(text("""
                    ...
                    SELECT
                        ...
                        COALESCE(coletado_em::timestamptz, NOW()),
                        geometry
            ...
            logger.error(f"SIGEF upsert falhou ({e}) — usando append como fallback")
            with self.engine.connect() as conn:
                try:
                    conn.execute(text("DROP TABLE IF EXISTS parcelas_sigef_staging"))
                    conn.commit()
    ...
    def save_all_layers(self, layers: dict):
        ...
        for k, v in layers.items():
            ...
            if v is None or (hasattr(v, "empty") and v.empty):
                logger.info(f"'{table}': layer vazio, dados existentes preservados")
                continue

            if table in _REFERENCE_LAYERS:
                self.save_geodataframe(v, table, if_exists="replace")
            elif (
                table == "parcelas_sigef"
                and isinstance(v, gpd.GeoDataFrame)
                and "codigo_imovel" in v.columns
            ):
    ...
    def upsert_processo(self, dados: dict) -> Optional[int]:
        with self.engine.connect() as conn:
                ...
                conn.execute(
                    text("""
                        UPDATE processos SET
                            tribunal          = COALESCE(NULLIF(:tribunal, ''), tribunal),
            ...
            r = conn.execute(
                text(
                    f"INSERT INTO processos ({','.join(campos)}) "
                    f"VALUES ({','.join(':'+c for c in campos)}) RETURNING id"
                ),
                vals,
            )
            pid = r.fetchone()[0]
    ...
    def save_score(self, processo_id: int, score_dict: dict):
        with self.engine.connect() as conn:
            conn.execute(
                text("DELETE FROM score_pericial WHERE processo_id=:id"),
                {"id": processo_id},
            )
            campos = [
            ...
            conn.execute(
                text(
                    f"INSERT INTO score_pericial ({','.join(campos)}) "
                ...
                text("""
                    ...
                    WHERE processo_id = :pid
                      ...
                      AND (
                          ...
                          OR (data_movimentacao IS NULL AND :dt IS NULL)
                      )
                    LIMIT 1
                """),
                {
            ...
            conn.execute(
                text("""
                    INSERT INTO movimentacoes
                        (processo_id, data_movimentacao, descricao, fonte, score_evento)
    ...
    def save_portarias(self, portarias: list):
        ...
        try:
            existing = self.query(
                "SELECT titulo, data_publicacao::text AS data_publicacao, fonte "
                "FROM portarias_diario_oficial"
            )
            if not existing.empty:
                existing_keys = set(existing.apply(_chave, axis=1))
                df_new["_key"] = df_new.apply(_chave, axis=1)
                df_new = df_new[~df_new["_key"].isin(existing_keys)].drop(
                    columns=["_key"]
                )
        except Exception as e:
            logger.warning(f"Dedup portarias: {e} — inserindo sem filtro")

        if df_new.empty:
            logger.info("Portarias: nenhuma nova (todas já existem no banco)")
            return

        df_new.to_sql(
            "portarias_diario_oficial", self.engine, if_exists="append", index=False
        )
                ...
                text("""
                    INSERT INTO peritos_agronomos
                        (nome, registro_profissional, especialidades,
                         municipios_atuacao, regiao_imea)
                    VALUES (:nome, :registro, :especialidades, :municipios, :regiao)
                    RETURNING id
                """),
                {
                    "nome":          dados.get("nome"),
    ...
    def stats(self, regiao: Optional[str] = None) -> dict:
        mun_f    = ""
        desap_f  = "WHERE desapropriacao_flag=TRUE"
        port_f   = ""
        assent_f = ""
        proc_f   = ""
        ...
        if regiao:
            mun_f    = "WHERE municipio IN (SELECT nome FROM municipios_mt WHERE regiao_imea = :regiao)"
            desap_f  = "WHERE desapropriacao_flag=TRUE AND municipio IN (SELECT nome FROM municipios_mt WHERE regiao_imea = :regiao)"
            port_f   = "WHERE municipio IN (SELECT nome FROM municipios_mt WHERE regiao_imea = :regiao)"
            assent_f = "WHERE municipio IN (SELECT nome FROM municipios_mt WHERE regiao_imea = :regiao)"
            proc_f   = "WHERE regiao_imea = :regiao"
            sc_f     = (
                "JOIN processos p ON score_pericial.processo_id = p.id "
                "WHERE p.regiao_imea = :regiao AND faixa_probabilidade="
            )
        ...
        for k, s in qs.items():
            try:
                result[k] = self.query(s, {"regiao": regiao}).iloc[0, 0]
            except Exception:
                result[k] = 0
        return result
...
def init_db():
    ...
    Deve ser chamado UMA VEZ no startup do FastAPI (lifespan).
    """
    db = Database()
    db._init_schema()
